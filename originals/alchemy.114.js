...
var game = {
        ...
        initProgress: function() {
            var a, b, c, d;
            for (game.history = JSON.parse(storage.getHistory()), game.progress = [], game.hiddenElements = [], game.progressWithoutFinal = [], a = 0, b = game.history.parents.length; b > a; a++)
                for (c = workspace.sex(game.history.parents[a]), d = 0; d < c.length; d++) bases.base[c[d]].hasOwnProperty("hidden") ? game.hiddenElements.push(c[d]) : -1 === game.progress.indexOf(c[d]) && -1 === game.prime.indexOf(c[d]) && (game.progress.push(c[d]), game.checkIfFinal(c[d]) || game.progressWithoutFinal.push(c[d]));
        ...
        onChildCreated: function() {
            $(document).on("childCreated", function(a, b, c) {
                var d, e;
                for (d = 0; d < b.length; d++) game.checkIfNotAlreadyDone(c) && (e = (new Date).getTime(), game.history.parents.push(c), game.history.date.push(e), $(document).trigger("updateHistory", [c, e]), bases.base[b[d]].hasOwnProperty("hidden") && ($(document).trigger("hiddenElementCreated", [b[d]]), -1 === game.hiddenElements.indexOf(b[d]) && game.hiddenElements.push(b[d]))), -1 !== game.progress.indexOf(b[d]) || -1 !== game.prime.indexOf(b[d]) || bases.base[b[d]].hasOwnProperty("hidden") || (game.progress.push(b[d]), game.checkIfFinal(b[d]) ? game.finalElements.push(b[d]) : game.progressWithoutFinal.push(b[d]), game.changeProgressCounter(), $(document).trigger("newChildCreated", [b[d]]))
            })
        },
        ...
        checkStructure: function() {
            ...
            for (a in bases.base) {
                if (bases.base[a].hasOwnProperty("platforms")) {
                    var b = bases.base[a].platforms.join(","); - 1 === b.indexOf(game.platform) && (delete bases.base[a], delete bases.names[a])
                }
        ...
        getFinalElements: function() {
            var a, b;
            for (a = 0, b = game.progress.length; b > a; a++) game.checkIfFinal(game.progress[a]) && -1 === game.finalElements.indexOf(game.progress[a]) && game.finalElements.push(game.progress[a])
        },
            ...
            for (b = 0, c = e.length; c > b; b++)
                if (bases.base[e[b]].hasOwnProperty("parents"))
                    for (d = 0; d < bases.base[e[b]].parents.length; d++)
                        if (-1 !== bases.base[e[b]].parents[d].indexOf(a)) return !1;
            return !0
        },
        checkMaxConnections: function() {
            game.maxConnections = 0;
            var a;
            for (a in bases.base) bases.base[a].hasOwnProperty("parents") && (game.maxConnections += bases.base[a].parents.length)
        },
        checkIfNotAlreadyDone: function(a) {
            for (var b = [Math.min(a[0], a[1]), Math.max(a[0], a[1])], c = 0, d = game.history.parents.length; d > c; ++c)
        ...
        toggleFullScreen: function() {
            var a = window.document,
                b = a.documentElement,
                c = b.requestFullscreen || b.mozRequestFullScreen || b.webkitRequestFullScreen || b.webkitRequestFullscreen || null,
                d = a.exitFullscreen || a.mozCancelFullScreen || a.webkitExitFullscreen || a.msExitFullscreen || null;
            a.fullscreenElement || a.mozFullScreenElement || a.webkitFullscreenElement ? d.call(a) : c.call(b)
        },
        ...
        checkHistoryIntegrity: function(a) {
            var b, c, d, e, f = [],
                g = function(a) {
                    return bases.base.hasOwnProperty(a) && (bases.base[a].hasOwnProperty("prime") || -1 !== f.indexOf(a)) ? !0 : !1
                };
            for (b = 0, c = a.parents.length; c > b; b++) Array.prototype.push.apply(f, workspace.sex(a.parents[b]));
    ...
    bases = {
        ...
        load: function() {
            bases.initEvents(), $.when($.ajax({
                ...
                url: localization.getURL("names.114.json"),
                type: "GET",
                dataType: "json",
                success: function(a) {
                    bases.names = a
                ...
                console.log("fail", a)
            }), $.getJSON("resources/images.114.json", function(a) {
                bases.images = a, bases.imagesLoaded = !0, $(document).trigger("imagesLoaded")
            })
        },
        initEvents: function() {
            $(document).on("languageChanged", function() {
                $.getJSON(localization.getURL("names.114.json"), function(a) {
    ...
    loadingScreen = {
        ...
        animateProgress: function() {
            if (loadingScreen.progressBar.innerHTML = loadingScreen.shownProgress + "%", loadingScreen.path.getTotalLength) {
                var a = loadingScreen.path.getTotalLength();
                loadingScreen.path.style.strokeDashoffset = navigator.userAgent.toLowerCase().indexOf("firefox") > -1 && navigator.oscpu.toLowerCase().indexOf("windows nt 6.1") > -1 ? Math.round((a - loadingScreen.progress / 100 * a) / 12) + "px" : a - loadingScreen.progress / 100 * a
            }
        },
        ...
        messages: {
            ...
            choose: function() {
                var a, b = loadingScreen.messages.getLevel(),
                    c = [];
                for (i = 0; i < loadingScreen.messages.list[b].length; i++) - 1 === loadingScreen.messages.shown.indexOf(loadingScreen.messages.list[b][i].id) && c.push(loadingScreen.messages.list[b][i]);
            ...
            getLevel: function() {
                for (var a = 0; a < loadingScreen.messages.list.length; a++)
                    for (var b = 0; b < loadingScreen.messages.list[a].length; b++)
                        if (-1 === loadingScreen.messages.shown.indexOf(loadingScreen.messages.list[a][b].id)) return a
            },
            probabilityChoose: function(a) {
                for (var b = 0, c = 0, d = loadingScreen.messages.list[a].length; d > c; c++) b += loadingScreen.messages.list[a][c].P;
                var e = Math.floor(Math.random() * b),
                    f = -1;
                for (c = 0; e >= c;) f++, c += loadingScreen.messages.list[a][f].P;
        ...
        initContent: function() {
            settings.offContent(), $("#settingsCheckAlreadyCombined").prop("checked", window.settings.data.checkAlreadyCombined), $("#settingsMarkFinalElements").prop("checked", window.settings.data.markFinalElements), $("#settingsHideFinalElements").prop("checked", window.settings.data.hideFinalElements), $("#settingsTurnOffNotifications").prop("checked", window.settings.data.turnOffNotifications), $("#settingsTurnOffElementsPositions").prop("checked", !window.settings.data.saveElementsPositions), $("#settingsHideElementNames").prop("checked", window.settings.data.hideElementsNames), $("#settingsNightMode").prop("checked", window.settings.data.nightMode), $("#settingsResetProgress").on("click", function(a) {
                ...
                window.settings.data.markFinalElements = this.checked, window.library.markFinalElements(this.checked), window.settings.data.markFinalElements ? workspace.$el.find(".element").each(function(a, b) {
                    -1 !== game.finalElements.indexOf(parseInt(b.getAttribute("data-elementId"), 10)) && (b.className += " finalElement")
    ...
    localization = {
        ...
        characters: {
            ...
            pl: "aÄ…bcÄ‡deÄ™fghijklÅ‚mnÅ„oÃ³pqrsÅ›tuvwxyzÅºÅ¼",
            de: "aÃ¤bcdefghijklmnoÃ¶pqrsÃŸtuÃ¼vwxyz",
            fr: "aÃ Ã¢Ã¦Ã¤bcÃ§deÃ©Ã¨ÃªÃ«fghiÃ®Ã¯jklmnoÃ´Å“Ã¶pqrstuÃ¹Ã»Ã¼vwxyÃ¿z",
            es: "aÃ¡bcdeÃ©fghiÃ­jklmnÃ±oÃ³pqrstuÃºÃ¼vwxyz",
            it: "aÃ bcdeÃ¨Ã©fghiÃ¬jklmnoÃ²pqrstuÃ¹vwxyz",
            nl: "aÃ¤bcdeÃ«Ã¨Ã©fghiÃ¯Ä³jklmnoÃ¶pqrstuÃ¼vwxyz",
            no: "aÃ¥Ã¦bcdefghijklmnoÃ¸pqrstuvwxyz",
            pt: "aÃ¡Ã¢Ã£Ã¡bcÃ§deÃ©ÃªfghiÃ­jklmnoÃ³Ã´ÃµpqrstuÃºvwxyz",
            sv: "aÃ¤Ã¥bcdefghijklmnoÃ¶pqrstuvwxyz"
        },
        loaded: !1,
        init: function() {
        ...
        checkLanguage: function() {
            ...
            for (var c in localization.domainMap)
                if (-1 !== localization.domainMap[c].indexOf(b)) {
        ...
        loadResources: function() {
            $.getJSON(localization.getURL("languagePack.114.json"), function(a) {
        ...
        compareLetter: function(a, b) {
            ...
            return 0 > c || 0 > d || c === d ? 0 : d > c ? -1 : 1
        },
        compare: function(a, b) {
            a = a.replace(localization.regex, ""), b = b.replace(localization.regex, "");
            for (var c, d = Math.min(a.length, b.length), e = 0; d > e;) {
    ...
    loading = {
        ...
        getModificationDate: function(a) {
            ...
            b.open("HEAD", a, !0), b.onreadystatechange = function() {
    ...
    GoogleAPI = {
        ...
        initEvents: function() {
            GoogleAPI.$login.on("click", function() {
                ...
                var a = localization.get(GoogleAPI.logged ? "login-logout" : "login-login");
        ...
        handleAuth: function(a) {
            GoogleAPI.checked = !0, !a || a.error || GoogleAPI.logged ? $(document).trigger(GoogleAPI.logged ? "loggedOut" : "notLoggedIn") : (GoogleAPI.logged = !0, GoogleAPI.accessToken = a.access_token, storage.setAuthUser(a.authuser), GoogleAPI.loadClient(), $(document).trigger("loggedIn"))
        },
        loadClient: function() {
    ...
    gestures = {
        ...
        longPress: {
            ...
            move: function(a) {
                if (-1 !== gestures.longPress.initPosition.x && -1 !== gestures.longPress.initPosition.y) {
                    var b = a.type;
...
window.storage = {
    ...
    checkOldVersionHistory: function() {
        if (null !== window.localStorage.getItem("history") && 0 != window.localStorage.getItem("history").length && null === window.localStorage.getItem("progress")) {
            for (var a = JSON.parse(window.localStorage.getItem("history")), b = {
                    parents: [],
                    date: []
                }, c = 0, d = a.parents.length; d > c; c++) b.parents.push([Math.min(a.parents[c][0] + 1, a.parents[c][1] + 1), Math.max(a.parents[c][0] + 1, a.parents[c][1] + 1)]), b.date.push(a.date[c]);
    ...
    checkImportedHistory: function() {
        if (null !== window.localStorage.getItem("history") && null !== window.localStorage.getItem("progress")) {
            for (var a, b = JSON.parse(window.localStorage.getItem("history")), c = 0, d = b.parents.length; d > c; c++) a = [Math.min(b.parents[c][0] + 1, b.parents[c][1] + 1), Math.max(b.parents[c][0] + 1, b.parents[c][1] + 1)], game.checkIfNotAlreadyDone(a) && (game.history.parents.push(a), game.history.date.push(b.date[c]));
...
var search = {
        ...
        initEvents: function() {
            $(document).on("searchAlphabethClick", function() {
                search.$alphabet.is(":visible") ? (search.$alphabet.hide(), search.activeLetter = null, null === search.activeCategory ? (library.refresh(), iscrollLibrary.scrollTo(0, 0, 500)) : library.show(library.showCategory(search.activeCategory))) : (el.show(), iscrollAlphabeth.refresh()), iscrollAlphabeth.refresh()
            })
        },
        initSearchBar: function() {
            ...
            $(document).keydown(function(b) {
                if (menu.$el.is(":visible") || 0 != $(":focus").length && $(":focus")[0] !== a && "INPUT" === $(":focus")[0].tagName || (b.keyCode >= 65 && b.keyCode <= 90 || 32 == b.keyCode) && (!b.ctrlKey || b.ctrlKey && b.altKey) && (a.type = "text", a.focus(), "undefined" != typeof iscrollLibrary ? iscrollLibrary.scrollTo(0, 0, 300) : initIScroll()), 8 === b.keyCode) {
                    ...
                    c = "INPUT" === d.tagName.toUpperCase() && ("TEXT" === d.type.toUpperCase() || "PASSWORD" === d.type.toUpperCase() || "FILE" === d.type.toUpperCase()) || "TEXTAREA" === d.tagName.toUpperCase() ? d.readonly || d.disabled : !0, c && b.preventDefault()
                }
        ...
        getNoElementsBefore: function(a) {
            for (var b = library.sortedProgress, c = 0, d = b.length; d > c;) {
                if (localization.compareLetter(a, bases.names[b[c]][0]) <= 0) return c;
                c++
            }
            return c - 1
        }
    ...
    update = {
        ...
        mergeHistoryData: function(a) {
            ...
            if (a.parents.length > 0)
                if (0 === game.history.parents.length) storage.setReset(a.reset), delete a.reset, game.history = a, c = !0;
                else {
                    for (var d = 0, e = a.parents.length; e > d; d++) game.checkIfNotAlreadyDone(a.parents[d]) && (game.history.parents.push(a.parents[d]), game.history.date.push(a.date[d]), c = !0);
                    b()
                } else b();
    ...
    library = {
        ...
        init: function() {
            ...
            var a = 5,
                b = window.setTimeout(function() {
                    library.addAll(), -1 === loadingScreen.list.indexOf("historySynchronized") && -1 === loadingScreen.list.indexOf("notLoggedIn") && loadingScreen.incrementProgress("notLoggedIn"), $(document).off("historySynchronized notLoggedIn")
                }, 1e3 * a);
            $(document).on("historySynchronized notLoggedIn", function() {
        ...
        addAll: function() {
            ...
            for (library.sortedProgress = d, a = 0, b = d.length; b > a; a++) library.add(d[a]);
        ...
        clear: function() {
            for (var a, b, c = library.el.getElementsByClassName("element"), d = 0, e = c.length; e > d; d++) a = $(c[d]), b = a.data("ptr"), b.draggable && b.draggable.destroy(), b.draggable = null, a.find("img").off(), a.data("ptr", null).off();
            library.el.innerHTML = ""
        },
        hideAll: function() {
            for (var a, b, c = library.el.getElementsByClassName("element"), a = 0, b = c.length; b > a; a++) c[a].style.display = "none"
        },
        reload: function() {
        ...
        reloadImages: function() {
            var a, b, c, d = library.el.getElementsByClassName("element"),
                e = function() {
                    var a = $(this);
                    a.data("ptr").draggable.changeHandles("img"), a.off("dragEnd", e)
                };
            for (a = 0, b = d.length; b > a; a++) d[a].firstChild.nextSibling.src = "data:image/png;base64," + bases.images[d[a].getAttribute("data-elementId")];
            for (a = 0, b = d.length; b > a; a++) d[a].firstChild.parentNode.removeChild(d[a].firstChild), d[a].firstChild.style.display = "block", c = $(d[a]).data("ptr").draggable, c.isDragging ? $(d[a]).on("dragEnd", e) : c.changeHandles("img")
        },
        markFinalElements: function() {
            var a, b, c;
            if (window.settings.data.markFinalElements && !window.settings.data.hideFinalElements)
                for (c = game.finalElements, a = 0, b = c.length; b > a; a++) library.el.querySelector('.element[data-elementid="' + c[a] + '"]').className += " finalElement";
            else
        ...
        showLetter: function(a) {
            ...
            for (b = 0, c = e.length; c > b; b++) bases.names[e[b]][0] === a && d.push(e[b]);
            return d
        },
        ...
        showLetterAndCategory: function(a, b) {
            var c, d, e = [];
            for (b = library.showCategory(b), c = 0, d = b.length; d > c; c++) bases.names[b[c]][0] === a && e.push(b[c]);
            return e
        },
        showPhrase: function(a) {
            for (var b, c, d = [], e = "[]{}'/\\()?<>+.*^$", f = game.prime.concat(game.progress), b = 0; b < e.length; b++) a = a.split(e[b]).join("");
            var g = new RegExp(a);
            for (b = 0, c = f.length; c > b; b++) g.test(bases.names[f[b]].toLowerCase()) && d.push(f[b]);
            return d
        },
        sort: function(a, b) {
        ...
        sortAlphabetically: function(a) {
            var b, c, d, e = {},
                f = [],
                g = [];
            for (b = 0, c = a.length; c > b; b++) d = bases.names[a[b]] + "_" + a[b], e[d] = a[b], f.push(d);
            for ("en" !== localization.language ? f.sort(localization.compare) : f.sort(), b = 0, c = f.length; c > b; b++) g.push(e[f[b]]);
            return g
        },
        sortByTime: function() {
            var a, b, c = {},
                d = [],
                e = [];
            for (a = 0, b = game.history.parents.length; b > a; a++) {
                var f = workspace.sex(game.history.parents[a]);
                c[game.history.date[a]] = f, d.push(game.history.date[a])
            }
            for (d.sort(), a = 0, b = d.length; b > a; a++) e = $.merge(e, c[d[a]]);
            return e
        },
        checkIfHideFinal: function(a) {
            return settings.data.hideFinalElements ? -1 === game.finalElements.indexOf(a) ? !1 : !0 : !1
        },
...
window.addEventListener("orientationChange", library.refreshIScroll, !1), LibraryBox.prototype.createElement = function(a) {
    ...
    b.setAttribute("class", "element"), b.setAttribute("data-elementType", "libraryBox"), b.setAttribute("data-elementId", this.id), b.style.display = a ? "none" : b.style.display, -1 !== game.finalElements.indexOf(this.id) && settings.data.markFinalElements && (b.className += " finalElement");
    var c, d;
...
var workspace = {
    ...
    clear: function() {
        elements = workspace.$el.find(settings.data.markFinalElements && workspace.$el.find(".finalElement").length > 0 ? ".finalElement" : ".element"), workspace.clearSpecified(elements), $(document).trigger("workspaceCleared")
    },
    clearSpecified: function(a) {
        for (var b, c, d = 0, e = a.length; e > d; d++) b = $(a[d]), c = b.data("ptr"), c && (c.droppable && c.droppable.destroy(), c.draggable && c.draggable.destroy(), c.draggable = null, c.droppable = null), b.find("img").off(), b.data("ptr", null).off().remove()
    },
    del: function(a) {
    ...
    sex: function(a) {
        ...
        for (b = 0, c = g.length; c > b; b++)
            for (e = workspace.ifSex(a, g[b]), d = 0; e > d; d++) f.push(parseInt(g[b], 10));
        return f
    },
    ifSex: function(a, b) {
        var c, d = 0;
    ...
    calculateOffspringPosition: function(a, b, c) {
        var d, e = function(c) {
            ...
            f = [],
            g = Math.floor(c / 2);
        for (d = -g; g >= d; d++)(0 !== d || c % 2) && f.push(e(d));
        return f
    },
    alreadyCombined: function(a) {
        return settings.data.checkAlreadyCombined ? game.checkIfNotAlreadyDone(a) ? !1 : !0 : !1
    },
    ...
    loadFromStorage: function() {
        if (settings.data.saveElementsPositions) {
            ...
            if (null !== a)
                for (var b = 0, c = a.length; c > b; b++)(-1 !== game.progress.indexOf(a[b].id) || -1 !== game.prime.indexOf(a[b].id)) && workspace.add(a[b].id, a[b].position)
        }
    },
    elementsNamesVisibility: function() {
        for (var a, b = workspace.$el.find(".element"), c = null, d = 0, e = b.length; e > d; d++) a = $(b[d]), c = a.data("ptr"), settings.data.hideElementsNames ? (c.hideElementName(), a.find(".elementName")[0].style.opacity = 0) : (c.removeHideElementName(), a.find(".elementName")[0].style.opacity = 1)
    }
};
WorkspaceBox.prototype.createElement = function(a) {
    ...
    c.style.opacity = 0, c.src = bases.imagesLoaded ? "data:image/png;base64," + bases.images[this.id] : "data:image/png;base64," + library.loadingImage, c.alt = bases.names[this.id], c.onload = function() {
        this.style.opacity = 1, c.onload = null
    };
    ...
    this.droppable = new Droppable(this.$el[0], {
        acceptOne: !0
    }), this.$el.on("droppableOver", function() {
        a.onOver()
    }), this.$el.on("droppableOut", function() {
        a.onOut()
